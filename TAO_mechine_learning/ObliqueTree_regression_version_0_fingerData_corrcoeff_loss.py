# -*- coding: utf-8 -*-
"""
Created on Sun Oct 10 08:48:44 2021
...
#%%
BCI_Data_Sub_10 = loadmat("Project_INL\data\BCImoreData_Subj_10_200msLMP.mat")
BCI_Data_Sub_10_t = loadmat("Project_INL\data\BCImoreData_Subj_10_200msLMPTest.mat")
Data_Feature_sub_10 = BCI_Data_Sub_10['Data_Feature']
SmoothedFinger_sub_10 = BCI_Data_Sub_10['SmoothedFinger']

Data_Feature_sub_10_t = BCI_Data_Sub_10_t['Data_Feature']
SmoothedFinger_sub_10_t = BCI_Data_Sub_10_t['SmoothedFinger']

Data_Feature_sub_10 = np.transpose(Data_Feature_sub_10)
Data_Feature_sub_10_t = np.transpose(Data_Feature_sub_10_t)
Data_Feature_sub_10_train = Data_Feature_sub_10[:,:]
Data_Feature_sub_10_test = Data_Feature_sub_10_t[:,:]
SmoothedFinger_sub_10_train = SmoothedFinger_sub_10[:,0]
SmoothedFinger_sub_10_test = SmoothedFinger_sub_10_t[:,0]


X_train_o = Data_Feature_sub_10_train
#X_train_2 = Data_Feature_sub_10_train + 1
X_test_o = Data_Feature_sub_10_test
y_train = SmoothedFinger_sub_10_train
#X_train = np.concatenate((X_train_1, X_train_2), axis=0)
#y_train = np.concatenate((y_train_1, y_train_1), axis=0)
y_test = SmoothedFinger_sub_10_test
...
X_train_o, X_test_o = scaler.transform(X_train_o), scaler.transform(X_test_o)

# ------------------- feature selection -----------------------
X = np.concatenate((X_train_o, y_train[:,None]), axis = 1)
print(np.corrcoef(X.T))
temp = abs(np.corrcoef(X.T)[-1,:-1])
ind = np.argpartition(temp,-30)[-30:]

X_train = X_train_o[:,ind]
X_test = X_test_o[:,ind]

'''
scaler = StandardScaler()
scaler.fit(y_train[:,None])
y_train, y_test = scaler.transform(y_train[:,None]), scaler.transform(y_test[:,None])
y_train, y_test = np.ravel(y_train), np.ravel(y_test)
'''

X_train = get_spikes_with_history(X_train,bins_before,bins_after,bins_current)
X_train_copy = X_train[bins_before:,:,:].copy()
X_train, y_train = X_train[bins_before:,:,:].reshape(X_train_copy.shape[0], X_train_copy.shape[1]*X_train_copy.shape[2]), y_train[bins_before:,]

X_test = get_spikes_with_history(X_test,bins_before,bins_after,bins_current)
X_test_copy = X_test[bins_before:,:,:].copy()
X_test, y_test = X_test[bins_before:,:,:].reshape(X_test_copy.shape[0], X_test_copy.shape[1]*X_test_copy.shape[2]), y_test[bins_before:,]


#%%
'''
#data_train = loadmat("project_INL\data\BCImoreData_Subj_10_200msLMP.mat")
...
class Node:
    ...
    def __init__(self, left=None, right=None, parent = None, *,split_func=None, indicator=None): 
        ...
        #self.split_func = LinearRegression()
        #self.split_func = Ridge(alpha=1e100)
        self.split_func = svm.SVR(kernel='rbf', C=1)
        # self.split_func = svm.SVC(kernel='linear')
        self.predictor = 0 # when splitfunction can not be used

    def check_split(self):
        if not self.is_leaf_node():
            self.split_func = LogisticRegression(C=1e-4, max_iter = 1000)
...
class tree:
    ...
    def predict(self, X_test):
        predictions = self._tree_predict(X_test,self.root)
        return predictions
    ...
    def _pre_train_tree(self, X, y, X_test, y_test):
        ...
        while len(nodes)>0:
            ...
            if n_temp.left is not None:
                if len(X_temp)>0 and len(np.unique(y_temp)) > 1:
                    ...
                    #print('--- X_temp.shape ')
                    #print(X_temp.shape)
                    dummy_pretrain_label_internal = np.random.randint(0,2,size=(len(y_temp),))
                    ...
                    X_all.append(X_temp[dummy_pretrain_label_internal == 0])
                    y_all.append(y_temp[dummy_pretrain_label_internal == 0])
                    # the second half is the dataset who have pred == 1 
                    X_all.append(X_temp[dummy_pretrain_label_internal == 1])
                    y_all.append(y_temp[dummy_pretrain_label_internal == 1])
        ...
        print("correlation coefficency:", np.corrcoef(np.concatenate((y_test[None,:], test_pred[None,:]), axis=0))[0][1])
    #####################################################################################
    #                                  tree prediction                                  # 
    #####################################################################################
    # trancerse tree 
    def _tree_predict(self, X, node):
        pred_all =[]
        for x in X: 
            pred_single = np.array(self._pred_traverse_tree(x, node))[0]
        ...
        while inter < iterations:
            ...
            while len(nodes) > 0:
                ...
                if n_temp.left is not None:
                    ...
                    left_error = self._pred_error(y_temp, left_internal_prediction_copy)
                    right_error = self._pred_error(y_temp, right_internal_prediction_copy)
                    #print(left_error.shape)
                    #print(right_error.shape)
                    ...
                    print(self._pred_error_corrcoef(y_temp, left_internal_prediction_copy).shape)
                    print(np.unique(self._pred_error_corrcoef(y_temp, left_internal_prediction_copy)))
                    print("--- right_correlation:")
                    print(self._pred_error_corrcoef(y_temp, right_internal_prediction_copy).shape)
                    print(np.unique(self._pred_error_corrcoef(y_temp, right_internal_prediction_copy)))
                    '''
                    internal_label_train = left_error - right_error # 1 left, 0 don't care, -1 right
                    ...
                    internal_label_train_copy[internal_label_train < 0] = 0 # left better
                    #print("left_better: " + str(len(internal_label_train_copy[internal_label_train < 0])))
                    internal_label_train_copy[internal_label_train > 0] = 1 # right better 
                    #print("right_better: " + str(len(internal_label_train_copy[internal_label_train > 0])))
            ...
            print("correlation coefficency (test):", np.corrcoef(np.concatenate((y_test[None,:], test_pred[None,:]), axis=0))[0][1])
            print("correlation coefficency (train):", np.corrcoef(np.concatenate((y_train[None,:], train_pred[None,:]), axis=0))[0][1])
    def _pred_error(self, y_true, y_pred):
        loss_all = []
    ...
    def _pred_error_corrcoef(self, y_true, y_pred):
        loss_all = []
        for i in range(len(y_true)):
            loss = np.corrcoef(np.concatenate((y_true[None,i], y_pred[None,i]), axis=0))
            loss_all.append(loss)
        return np.array(loss_all)
...
# -------------------- helper functions for oblique tree forest ----------------------
def pred_error_corrcoef(y_true, y_pred):
    loss = np.corrcoef(np.concatenate((y_true[None,:], y_pred[None,:]), axis=0))
    return loss

def bagging_predict(trees, X_test):
    predictions = [one_tree.predict(X_test) for one_tree in trees]
    
    return max(set(predictions), key=predictions.count)

def ObliqueTree_forest(forest_size, X_train, y_train, X_test, y_test, max_train_iterations, max_tree_depth):
    trees = list()
    for i in range(forest_size):
        one_tree = tree(max_depth = max_tree_depth)
        one_tree.fit(X_train, y_train, X_test, y_test, max_train_iterations)
    trees.append(tree)
...
print("---------- random forest ------------")
Random_forest_test = RandomForestRegressor(n_estimators=10, max_depth=3)
Random_forest_test.fit(X_train, y_train)
Y_predict = Random_forest_test.predict(X_test)
corrcoef = pred_error_corrcoef(y_test, Y_predict)
print(corrcoef[1][0])


#%% Decision Tree 
print("------------- Decision Tree -------------")
Decision_Tree = DecisionTreeRegressor('mse', max_depth=3)
...
#%% Oblique Tree forest 
print("----------------- adaBoost ---------------")
# predictions = ObliqueTree_forest(forest_size, X_train, y_train, X_test, y_test, max_train_iterations, max_tree_depth):
AdaBoost_Forest = AdaBoostRegressor(n_estimators = 10, loss = 'linear', learning_rate = 0.5)
...
#%% SVM
split_func = svm.SVR(kernel='rbf', C=1)
split_func.fit(X_train, y_train)
Y_predict = split_func.predict(X_train)
corrcoef = pred_error_corrcoef(y_train, Y_predict)
print(corrcoef[1][0])
Y_predict = split_func.predict(X_test)
corrcoef = pred_error_corrcoef(y_test, Y_predict)
print(corrcoef[1][0])
